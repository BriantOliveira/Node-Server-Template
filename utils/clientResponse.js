 ...
 * });
 */
function respondWith(res, code, message, data) {
  const payload = {
    status: StatusCodes[code],
    message: message || [StatusMessages[code]],
  };
 ...
 * @param {Express.Response} res An Express Response object
 * @param {number} code An HTTP Status Code
 * @param {Object} opts An options object that accepts 'message', and 'data'.
...
const clientResponse = (res, code, opts) => {
  ...
  if (opts) {
    ...
    payload.message = opts.message || StatusMessages[code];
